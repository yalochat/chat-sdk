...
class AudioViewModelTest {
    ...
    fun `initial state is not recording with Initial status`() {
        ...
        assertEquals(AMPLITUDE_DATA_POINTS, vm.state.value.audioData.amplitudes.size)
        assertEquals(AMPLITUDE_DATA_POINTS, vm.state.value.audioData.amplitudesPreview.size)
    }
    ...
    fun `StartRecording resets amplitudes to AMPLITUDE_DATA_POINTS default values`() = runTest(dispatcher) {
        ...
        assertEquals(AMPLITUDE_DATA_POINTS, vm.state.value.audioData.amplitudes.size)
        assertTrue(vm.state.value.audioData.amplitudes.all { it == DEFAULT_AMPLITUDE })
    }
