...
void main() {
  group(WaveformCompressor, () {
    ...
    test(
      'stretches a short recording across the full buffer so the tail does not stay flat',
      () {
        final compressor = WaveformCompressor(binCount: 4, defaultValue: -30);
    ...
    test(
      'returns a flat preview at the level of the only sample when a single sample has been pushed',
      () {
        final compressor = WaveformCompressor(binCount: 4, defaultValue: -30);
    ...
    test(
      'compacts older samples and doubles the stride when the buffer fills',
      () {
        final compressor = WaveformCompressor(binCount: 4, defaultValue: -30);
    ...
    test(
      'replaces the default when the first sample in a bin is quieter than the default',
      () {
        final compressor = WaveformCompressor(binCount: 4, defaultValue: -30);
