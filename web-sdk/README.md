...
| `sessionMode` | `"default" \| "differentSessionPerContext"` | No  | Controls how the chat session is partitioned. With `"differentSessionPerContext"`, the chat is partitioned by `openContext`: two tabs opened with the same `openContext` share a conversation, while different `openContext` values start a fresh conversation. With `"default"` or unset, `openContext` has no effect on session identity. |
