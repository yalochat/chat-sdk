...
| `sessionMode` | `"perUserId" \| "perContext"` | No  | Controls how the chat session is partitioned. With `"perContext"`, the chat is partitioned by `openContext`: two tabs opened with the same `openContext` share a conversation, while different `openContext` values start a fresh conversation. With `"perUserId"` (default) or unset, `openContext` has no effect on session identity. |
